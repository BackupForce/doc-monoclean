...
| 實體 | 類型 | 說明 | 關聯 |
|------|------|------|------|
| ResourceNode | Aggregate Root | 對應外部系統的實體資源（如 Tenant/Workspace），可階層化 | ResourceNode→Children（一對多） |
...
> 註：AccessControl 不擁有 `User`、`Tenant`、`Workspace` 的資料，只保存其對應的 `NodeId` 與外部自然鍵。
> NodeId 綁定 Directory 模組的 Workspace ResourceNode，作為授權繼承的節點。

### 2.2 Value Objects
- **PermissionCode**：`<module>.<resource>.<action>`，如 `directory.workspaces.manage`
...
| GrantPermissionCommand | Subject(UserId or RoleGroupId), PermissionCode, NodeId?, Decision | Result | 指派/覆寫授權，支援 Workspace 範圍節點 |
| RevokePermissionCommand | Subject, PermissionCode, NodeId? | Result | 撤銷授權（Workspace ResourceNode 為 NodeId） |
| GetEffectivePermissionsQuery | UserId, NodeId | Result<List<PermissionCode>> | 計算使用者於 Workspace 節點的有效權限（含繼承/拒絕） |
| CheckAccessQuery | UserId, NodeId, PermissionCode | Result<bool> | 快速檢查單一權限（NodeId = Workspace ResourceNodeId） |
...
> API 中的 `nodeId` 查詢或路徑參數皆指向 Workspace 的 ResourceNodeId。
...
- Directory 相關（由 Directory 模組提供權限定義、這裡註冊）：`directory.tenants.manage`, `directory.workspaces.manage`, `directory.members.invite`, `directory.members.manage`, `directory.workspaces.read`, `directory.positions.manage`, `directory.invitations.read`
...
> **冪等**：以外部自然鍵（TenantId/WorkspaceId/UserId）+ 事件版本號避免重複處理
...
- **Scope**：若無 `NodeId` 即為全域授權；節點授權只在 Workspace 分支內有效。
